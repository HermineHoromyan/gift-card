...
using NverCard.Public.Client.Pages.Thanks;
...
namespace NverCard.Public.Client.Pages.Checkout
{
    public partial class Checkout
    {
        ...
        /// Selected envelope
        /// </summary>
        public EnvelopeType SelectedEnvelope { get; set; } = EnvelopeType.NotSelected;
        ...
        private async Task CreateOrder()
        {
            try
            {
                ...
                await CreateCustomerOrderCommandProxy.Execute(new CreateCustomerOrderModel
                {
                    ...
                    DeliveryDateTime = WithDelivery ? SelectedDateTime : null,
                ...
                ButtonLoading = false;
                ConfirmOrder = true;
                await ShoppingCartService.GetCustomerShoppingCartAsync();
                thanks.OpenThanks();
                StateHasChanged();
            }
        ...
        private void SelectEnvelope(EnvelopeType type)
        {
            SelectedEnvelope = type;
        }
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            {
                await IToastService.Error("Անհայտ սխալ");
                throw;
            }
        ...
        protected int GetCartTotalPrice()
        {
            return (ShopingCartItems != null && ShopingCartItems.Count > 0) ?
                ...
                (SelectedEnvelope != EnvelopeType.White && SelectedEnvelope != EnvelopeType.NotSelected ? EnvelopePrice : 0) : 0;
        }
        ...
        private void ChangeDeliveryType(bool isWithDelivery)
        {
            ...
            if (!isWithDelivery)
            {
                ...
                SelectedEnvelope = EnvelopeType.NotSelected;
            }
            else
            {
                SelectedEnvelope = EnvelopeType.White;
            }
        ...
        private string GetEnvelopeActiveStyle(EnvelopeType type)
        {
            return SelectedEnvelope == type ? "active" : "";
